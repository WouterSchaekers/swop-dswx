...
{
	public void dispatchCast(CastIN treatment, DiagnoseIN Diagnose,
			UserManager userm, Warehouse warehouse, PatientFile file,
			TimeLord systemtime, TaskManager taskmanager)
		...
		UnscheduledCast newCast = new UnscheduledCast(file,
				(Diagnose) Diagnose, systemtime.getSystemTime(),
				(Treatment) treatment, userm, warehouse);
	...
	public void dispatchMedication(MedicationIN med, DiagnoseIN Diagnose,
			...
			InvalidHospitalDateException, InvalidTreatmentException,
			InvalidTimeSlotException, InvalidHospitalDateArgument {
		UnscheduledMedication m = new UnscheduledMedication(file,
				(Diagnose) Diagnose, systemtime.getSystemTime(),
				(Medication) med, userm, warehouse);
	...
	public void dispatchSurgery(SurgeryIN treatment, DiagnoseIN Diagnose,
			UserManager userm, MachinePool pool, PatientFile file,
			TimeLord systemtime, TaskManager taskmanager)
			throws InvalidResourceException, InvalidDurationException,
			InvalidOccurencesException, InvalidAmountException,
			InvalidHospitalDateException, InvalidTreatmentException,
			InvalidTimeSlotException, InvalidHospitalDateArgument {
		UnscheduledSurgery newSurg = new UnscheduledSurgery(file,
				(Diagnose) Diagnose, systemtime.getSystemTime(), userm, pool,
				(Surgery) treatment);
		((Diagnose) Diagnose).assignTreatment((Treatment) treatment);
