...
public class Task
{
	private Collection<Schedulable> myResources;
	private TimeSlot mySlot;
	/**
	 * @see Requirement
	 */
...
//	 */
	...
	public Task(Collection<Schedulable> usedResources, TimeSlot timeSlot){
		if(!canHaveAsResources(usedResources)){
			throw new IllegalArgumentException("Invalid resource passed to Task constructor!");
		}
		this.myResources = Task.cloneCollection(usedResources);
		this.mySlot = timeSlot;
	}
	
	private static Collection<Schedulable> cloneCollection(
			Collection<Schedulable> myResource) {
		Collection<Schedulable> res= new ArrayList<Schedulable>();
...
//	}
	 ...
	 */
	private boolean canHaveAsResources(Collection<Schedulable> s) {
