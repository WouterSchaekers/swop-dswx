...
public class TestScenarios
{
	...
	// maybe using strings as a key for patient files isn't the best way of doing things...
	// then again we prefer not passing any objects from the domain layer to the ui.
	
	/**
	 * A basic scenario where only one of each exists and only basic use cases are walked through.
	 * 
	 * <p><b><u>.::Excuting::.</u></b></p>
	 * <li> Initialise the hospital with empty constructor.
	 * <li> Hospital admin logs in.
	 * <li> Hospital admin adds a doctor, a nurse and an XRayScanner to the hospital.
	 * <li> Hospital admin logs out
	 * <li> Nurse Jenny logs in.
	 * <li> Nurse Jenny registers a new patient: Dieter Geboers.
	 * <li> Nurse Jenny asks the system for available doctors.
	 * <li> Nurse Jenny tells the system that she wants to schedule an appointment for Dieter Geboers with doctor Jonathan
	 * <br><br>Scenario ends here. 
	 */
	@Test
	public void scenario1() throws UserAlreadyExistsException, InvalidNameException, InvalidTimeSlotException, InvalidLoginControllerException, InvalidHospitalException, InvalidLocationException, InvalidSerialException, InvalidPatientFileOpenController {
		...
		AddHospitalEquipmentController ahec = new AddHospitalEquipmentController(lc);
	...
	/**
	 * <p>This scenario is the sequel to scenario 1. 2 new nurses have been added.
	 * There also are a blood analyser and an ultrasoundscanner available.</p>
	 * 
	 * <p>Doctor Jonathan has seen patient Dieter and has decided the best course
	 * of action is to experiment on him with 2 medical tests to determine the
	 * diagnose...</p>
	 * 
	 * <p><b><u>.::Excuting::.</u></b></p>
	 * <li> Doctor Jonathan logs in.
	 * <li>  Doctor Jonathan opens the patient file of Dieter.
	 * <li> Doctor Jonathan creates a bloodanalysis for Dieter that focusses on Infections and has 5 iterations.
	 * <li> Doctor Jonathan creates an x-ray scan for Dieter that takes 3 images of Dieter's head and has zoom level 2.2.
	 * <li> Doctor Jonathan logs out.
	 * <li> The hospital administrator logs in.
	 * <li> The hospital administrator advances the time by 10 hours.
	 * <li> The hospital admin logs out.
	 * <li> Nurse Jenna logs in.
	 * <li> Nurse Jenna requests a list of unfinished medical tests.
	 * <li> Nurse Jenna enters the result of the blood analysis of Dieter.
	 * <li> Nurse Jenna enters the result of the x-ray scan of Dieter.
	 * <li> Nurse Jenna logs out.
	 * <li> Doctor Jonathan logs in.
	 * <li> Doctor Jonathan consults the patient file of Dieter.
	 * <li> Doctor Jonathan enters the diagnose for Dieter: "Hypochondriasis".
	 * <li> Doctor Jonathan discharges Dieter.
	 * <li> Doctor Jonathan logs out.
	 * <br><br>
	...
	public void scenario2() throws UserAlreadyExistsException,
		...
		for(MedicalTestFactory mf : mtf) {
			if (mf instanceof BloodAnalysisFactory) {
				System.out.println(" Doctor Jonathan has specified the following focus: \"Infections\".");
				((BloodAnalysisFactory) mf).setFocus("Infections");
				System.out.println(" Doctor Jonathan has specified the following amount of analysises: 5");
				((BloodAnalysisFactory) mf).setNumberOfAnalysis(5);
				HospitalDate d = omtc.addMedicaltest(mf.create());
				System.out.println("Blood analysisis has been successfully created!");

				if(d == null)
					System.out.println("The bloodanalysis has not been scheduled yet.");
				else 
					System.out.println("The blood analysis was scheduled to go through on " + d);
			}		
		}
		
		System.out.println(" Doctor Jonathan requested to perform an x-ray scan on Dieter.");
		for(MedicalTestFactory mf : mtf) {
			if (mf instanceof XRayScanFactory) {
				System.out.println(" Doctor Jonathan has specified the following body part: \"Head\".");
				((XRayScanFactory) mf).setBodyPart("Head");
				System.out.println(" Doctor Jonathan has specified the following amount of images: 3");
				((XRayScanFactory) mf).numberOfNeededImages(3);
				System.out.println(" Doctor Jonathan has specified the following zoomlevel: 2.2");
				((XRayScanFactory) mf).setZoomLevel((float) 2.2);
				HospitalDate d = omtc.addMedicaltest(mf.create());
				System.out.println("X-ray scan has been successfully created!");

				if(d == null)
					System.out.println("The x-ray scan has not been scheduled yet.");
				else 
					System.out.println("The x-ray scan was scheduled to go through on " + d);
			}
		}
		...
		atc.setNewSystemTime(new HospitalDate(h.getSystemTime().getSystemTime().getTimeSinceStart() + HospitalDate.ONE_HOUR * 10));
		System.out.println("Success!");
